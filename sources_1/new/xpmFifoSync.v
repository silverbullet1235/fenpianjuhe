...
module xpmFifoSync #(
    parameter   WRITE_DATA_WIDTH    = 32        ,
                ...
                FIFO_DEPTH_WIDTH    = $clog2(FIFO_WRITE_DEPTH),
    ...
    output                              data_valid  , // 改动 
    output      [READ_DATA_WIDTH-1:0]   Dout        ,
    output                              fifo_full   ,
    output                              fifo_empty  ,
    output                              prog_empty  , // 改动

    output reg [FIFO_DEPTH_WIDTH-1:0]   fifo_count  
);
    

always @(posedge clk ) begin
    if( !rst_n )                    fifo_count <= 'd0               ;
    else if( rd_en && wr_en       ) fifo_count <= fifo_count        ;
    else if( rd_en && !fifo_empty ) fifo_count <= fifo_count - 1    ;
    else if( wr_en && !fifo_full  ) fifo_count <= fifo_count + 1    ;
